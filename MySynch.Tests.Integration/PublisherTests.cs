...
//            var data = sourceOfDataProxy.GetData(new GetDataRequest {FileName = @"C:\MySynch.Source.Test.Root\File1.xml"});
...
//            Assert.True(subscriber.ApplyChangePushItem(new ReceiveMessageRequest { PublisherMessage = publishedPackageRequestResponse.ChangePushItems[0], SourceRootName = publishedPackageRequestResponse.SourceRootName }).Success);
